...
def Variationer_learn(shots, l, m, proba=None, n=4, distri_size=0, easy=False):
    ...
    def Remp(theta):
        ...
        if distri_size == 0:
            input_db = Database[:m]
            input = input_var + input_db
            target = [0]*m + [1]*m
            index = [i for i in range(2*m)]
            r.shuffle(index)
            input = [[input[i]] for i in index]
...
#mosel()
#print(Variationer_learn(1000, 4, 32))
...
print( beta.pdf(arr, a, b), arr)
print(sum(beta.pdf(arr, a, b)*1/32))
